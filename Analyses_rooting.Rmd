...
## Clustering analysis on callus data
...
Here, we will use statistical analyses to test whether treatment or individual predict survival and heights of plantlets after 5-weeks of culture.

### Tidy the data

The analyses are based on `survheig`, more specifically on `survheig$X5_w_survival`. We need to start by extracting treatments from `survheig$Treatment` (which contains block, treatment and replicate information in one object).

```{r survtidy, echo=T, eval=T}
####~~~~
...
### Treatment and individual effects on plantlet survival

#### Test for normality

```{r normsurv, echo=T, eval=T}
# Test for normality with Shapiro-Wilk Normality Test
shapiro.test(survheig$X5_w_survival)
```

The test strongly suggests rejects normality of the survival data. We are therefore going to apply the same approach as with the callus data. Let's look at the distribution of the data to confirm that we can use the binomial family (Figure \@ref(fig:distrSurv)):

```{r distrSurv, echo=T, eval=T, fig.cap="Barplot of survival of plantlets after 5 weeks of growth."}
...
```{r survivalmodels, echo=T, eval=T}
...
We are not pursuing our investigations since both models suggest that there is no significant treatment effect on survival. This means that if the shoot tips were producing roots then they had equal probabilities of survival independently of treatment. 

### Effect of rooting clusters on plantlet heights

Here, we will only work on alive plantlets (`survheig$X5_w_survival` == 1) and conduct statistical analyses to test whether individuals from the different rooting clusters have different growth rates:
