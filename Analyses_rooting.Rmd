...
## Importing and tyding data {.tabset .tabset-pills}

### Overview of data

The objective of this section is to merge data on rooting and callus stored in spreadsheets (in `csv` format). Each spreadsheet corresponds to a block and the code below collate these into one `data.frame` for downstream analyses. The R code associated to data analyses is available in section \@ref(tidy). 

The final dataset (`MERGE` object) contains the following columns (see Table \@ref(tab:statssampling) for more details on sampling):
...
Please find below the analyses conducted on callus and rooting data.
