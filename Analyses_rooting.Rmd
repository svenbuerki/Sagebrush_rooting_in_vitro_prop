...
#How many shoot tips were transfered? 
#What are their survival and height (with standard dev.) @ 3 and 5 weeks
...
colnames(clustSurvHeig) <- c("Rooting cluster", "N", "3 w. surv. (%)", "3 w. plant height (cm)", "5 w. surv. (%)", "5 w. plant height (cm)")
...
#Plot table
knitr::kable(clustSurvHeig, row.names=F, caption = "Seedlings survival and heights sorted by rooting clusters.")
```
...
#Matrix summarizing effect of growth regulators on rooting
survheigTreat <- matrix(ncol=7, nrow=15)
colnames(survheigTreat) <- c("Cluster", "N","Growth regulator", "Concentration (mg/l)", "Survival (%)", "Height (cm)", "Treatment")
...
#Plot table
knitr::kable(survheigTreat, row.names=F, caption = "Seedlings survival and heights sorted by rooting clusters and treatments.")
...
```{r echo=F, eval=F}
x <- subset(survheigTreat, survheigTreat$TreatConc %in% c("IBA_05", "IBA_1") & survheig$Cluster == "blue" & survheig$X5_w_survival != "0")

boxplot(as.numeric(x$X3_w_height) ~ as.vector(x$SeedID))

boxplot(as.numeric(x$X3_w_height) ~ as.vector(x$SeedID), names = paste(names(seedID), as.vector(seedID), sep = "\n"), las=2, xlab = "", cex=.8)

seedID <- table(as.vector(x$SeedID))
