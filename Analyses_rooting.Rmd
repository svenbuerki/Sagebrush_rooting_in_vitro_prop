...
We are using Fisher tests to test whether treatments and rooting clusters had an effect on plantlets survival.

```{r fishertreatsurv, echo=F, eval=T}
###~~~
#Treatment effect on survival
##~~~
survTreat <- t(table(survheig$Treat, survheig$X5_w_survival))
print(survTreat)

#Conduct Fisher test
fisher.test(survTreat)

###~~~
#Cluster effect on survival
##~~~
survClust <- t(table(survheig$Cluster, survheig$X5_w_survival))
print(survClust)

#Conduct Fisher test
fisher.test(survClust)
```

We are not pursuing our investigations any further since the analyses suggest that there are no significant treatment and cluster effects on survival. This means that if shoot tips were producing roots then they had equal probabilities of survival independently of treatment and/or cluster. 
